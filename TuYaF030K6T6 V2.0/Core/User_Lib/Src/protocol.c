...
void weather_open_return_handle(unsigned char res, unsigned char err)
{
	...
	if(res == 1) {
		//打开天气返回成功
		HAL_UART_Transmit(&huart1, (unsigned char*)"Weather Success!", 16, 0xff);
	}else if(res == 0) {
		HAL_UART_Transmit(&huart1, (unsigned char*)"Weather Failed!", 15, 0xff);
		//打开天气返回失败
		//获取错误码
		//err_num = err;
	}
...
void weather_data_user_handle(char *name, unsigned char type, const unsigned char *data, char day)
{
	...
	if(type == 0) { //参数是INT型
		...
		sprintf(&value_string[0],"Weather:%d",value_int);
		HAL_UART_Transmit(&huart1, &value_string[0], 30, 0xff);
	}else if(type == 1) {
